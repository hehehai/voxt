...
安全约束：普通键盘键不能作为无修饰全局快捷键，例如单独的 `X` 不允许。普通键必须至少带一个修饰键，例如 `⌘X`、`fn+X`、`Control+X`。modifier-only 快捷键和中键及以上鼠标按键不受此规则影响。
...
- 如果同一个快捷键同时存在 `tap` 和 `doubleTap`，运行时会优先等待/识别 `doubleTap`。
...
普通键盘快捷键通过 `keyDown/keyUp` 处理。全局普通键盘快捷键必须至少带一个修饰键；裸字母/数字/功能键不会进入运行时路由，避免全局吞掉用户在其它 App 中的正常输入。
...
| 改写=`同一快捷键 doubleTap`，转录=`同一快捷键 tap`                   | 运行时 doubleTap 优先，单击不会立即触发 tap。设置页会对同一业务相同 hotkey 报重复；跨业务同 hotkey 不同行为不报同类重复。 |
