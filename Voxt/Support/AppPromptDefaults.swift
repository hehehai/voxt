...
enum AppPromptDefaults {
    ...
    private static func chineseSimplifiedText(for kind: AppPromptKind) -> String {
        ...
        case .enhancement:
            ...
            10. 若提供了当前 App 上下文或截图，仅可将其用于消解指代、识别当前界面目标，以及修正明显识别错误；不要机械复述界面上可见的文本。
            11. 若口述内容与 App 上下文冲突，默认优先口述内容；只有当上下文能明确消解“这个”“在这里回复”“发这个”之类的简短指代时，才参考上下文。
            12. 不要根据语音、文本上下文或截图中未明确出现的信息自行补全；如果截图内容模糊、残缺或有歧义，直接忽略该部分。
            13. 若清理后无有效内容，返回空字符串。
        ...
        case .translation:
            ...
            8. 若内容中有顺序列表相关表述，使用序号列表方式整理；若有并列关系且明确的非顺序类内容，使用无序列表“-”表示；若存在子列表，采用 Markdown 嵌套列表格式（子列表前加 4 个空格或 1 个制表符）。
            9. 适当位置增加换行间隔，使内容清晰有条理。
            10. 将整理后的内容准确翻译为 {{TARGET_LANGUAGE}}，保留原意，不得擅自增删信息。若目标语言指定了字形或书写变体，例如简体中文或繁体中文，必须严格使用该变体，不得混用。
            11. 若清理后无有效内容，返回空字符串。
    ...
    private static func japaneseText(for kind: AppPromptKind) -> String {
        ...
        case .enhancement:
            ...
            10. 現在の App コンテキストやスクリーンショットが提供された場合、それらは指示対象の特定、現在の UI ターゲットの把握、明らかな認識ミスの修正にのみ使ってください。画面上の可視テキストを機械的に最終結果へ書き写してはいけません。
            11. 発話内容と App コンテキストが衝突する場合は、原則として発話内容を優先してください。ただし、「これ」「ここに返信」「これを送る」のような短い指示を文脈が明確に特定できる場合のみ参考にしてください。
            12. 発話、テキストコンテキスト、スクリーンショットに明確に存在しない情報を推測で補わないでください。スクリーンショットの内容がぼやけている、欠けている、曖昧な場合はその部分を無視してください。
            13. 整理後に有効な内容が残らない場合は、空文字列を返してください。
        ...
        case .translation:
            ...
            8. 内容に順序付きリストに関する表現がある場合は、番号付きリストとして整理してください。明確な並列関係で順序を持たない内容がある場合は、「-」の箇条書きで表してください。サブリストがある場合は、子項目の前に半角スペース4個またはタブ1個を入れる Markdown のネストリスト形式を使用してください。
            9. 内容が明確で構造化されるように、適切な位置に改行を追加してください。
            10. 整理後の内容を {{TARGET_LANGUAGE}} へ正確に翻訳し、原意を保ち、情報を勝手に追加・削除しないでください。翻訳先言語で文字体系や表記変種が指定されている場合、たとえば簡体字中国語や繁体字中国語では、その指定どおりの表記だけを使用し、混在させないでください。
